...
export async function downloadSolcCompilers(versions, quiet) {
...
export async function getCompiler(version, { preferWasm, compilerPath }) {
...
async function getCompilerFromPath(compilerVersion, compilerPath) {
    log(`Using custom compiler ${compilerPath}`);
    if (!(await exists(compilerPath))) {
        throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY.COMPILER_PATH_DOES_NOT_EXIST, { compilerPath, version: compilerVersion });
    }
    ...
    if (match === null || match.groups === undefined) {
        throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY.PARSING_VERSION_STRING_FAILED, { versionString: stdout, compilerPath });
    }
...
async function getCompilerFromVersion(version, { preferWasm }) {
    if (!preferWasm) {
        const platform = CompilerDownloaderImplementation.getCompilerPlatform();
