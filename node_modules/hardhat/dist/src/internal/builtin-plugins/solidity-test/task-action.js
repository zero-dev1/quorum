...
const runSolidityTests = async ({ testFiles, chainType, grep, noCompile, verbosity, testSummaryIndex }, hre) => {
    ...
    const sourceNameToUserSourceName = new Map(edrArtifacts.map(({ userSourceName, edrArtifact }) => [
    ...
    const runStream = run(chainType, edrArtifacts.map(({ edrArtifact }) => edrArtifact), testSuiteIds, testRunnerConfig, tracingConfig, sourceNameToUserSourceName, options);
    let failed = 0;
    let passed = 0;
    let skipped = 0;
    let failureOutput = "";
